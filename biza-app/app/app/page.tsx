...
export default function AppDashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6">
      <div className="space-y-2 bg-forest text-white rounded-lg p-5 md:p-6 border border-transparent">
        ...
        {!user.onboardingComplete ? (
          <Button href="/onboarding" variant="primary" className="mt-2 w-full max-w-xs flex">
            Finish onboarding
          </Button>
        ) : null}
      </div>
      ...
      <Card className="space-y-3">
        ...
        {user.lastChapterId ? (
          <Button href="/courses/selling-online/dropshipping/learn" variant="outline" className="w-full max-w-xs mx-auto flex text-xs">
            Resume course
            <ArrowRight size={14} />
          </Button>
        ) : (
          <p className="text-sm text-stone text-center">No chapter in progress yet.</p>
        )}
      </Card>
      ...
      {!user.isPro ? (
        <Card hover={false} className="space-y-3 border-gold/30 bg-gold/5">
          ...
          <Button href="/app/upgrade" variant="primary" className="w-full max-w-xs mx-auto flex text-xs">
